			...
			for(int i = 0; i < n.params.size(); i++) {
				for(int j = i+1; j < n.params.size(); j++) {
					...
					if(p1 instanceof IntConcreteParam &&
						...
						if(((IntConcreteParam) p1).exp instanceof VarExp &&
								((IntConcreteParam) p2).exp instanceof VarExp)
							if(((VarExp) ((IntConcreteParam) p1).exp).name.
									equals(((VarExp) ((IntConcreteParam) p2).exp).name))
								reports.addErrorReport("Repeated integer parameter " +
										((VarExp) ((IntConcreteParam) p1).exp).name + 
